...
defmodule Monitorex.SmokeSeeder do
  ...
  defp reset_ets do
    ...
    Enum.each(@ets_tables, fn t ->
      ...
      try do
        :ets.new(t, [:public, :named_table, type, read_concurrency: true])
      rescue
        _ -> :ok
      end
    end)
  ...
  defp seed_outbound do
    ...
    # Host aggregates
    [
      {"api.example.com", 342, 12, 15_200.0, now - 60_000_000, "Finch"},
      {"cdn.example.com", 1_891, 3, 48_500.0, now - 120_000_000, "Tesla"},
      {"auth.example.com", 156, 8, 6_800.0, now - 300_000_000, "Finch"}
    ]
    |> Enum.each(fn {h, r, e, td, ls, c} ->
      :ets.insert(
        :monitorex_outbound_hosts,
        {h, %{requests: r, errors: e, total_duration: td, last_seen: ls, client: c}}
      )
    end)
    ...
    |> Enum.each(fn {host, list} ->
      list
      |> Enum.each(fn {path, r, e, td} ->
        :ets.insert(
          :monitorex_outbound_endpoints,
          {{host, path}, %{requests: r, errors: e, total_duration: td, last_seen: now}}
        )
      end)
    end)
    ...
    |> Enum.each(fn {key, r, e, td, ls} ->
      :ets.insert(
        :monitorex_inbound_routes,
        {key, %{requests: r, errors: e, total_duration: td, last_seen: ls}}
      )
    end)
    ...
    |> Enum.each(fn {name, r, e, td, ls} ->
      :ets.insert(
        :monitorex_inbound_consumers,
        {name, %{requests: r, errors: e, total_duration: td, last_seen: ls}}
      )
    end)
...
# ── Verify server is up ──
{:ok, 200, _, body} =
  :hackney.get("http://127.0.0.1:#{port}/monitorex/health", [], "", with_body: true)
